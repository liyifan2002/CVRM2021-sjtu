...
#ifndef _MSC_VER
#define VDefault __attribute__((visibility("default"))) 
#else
#define VDefault
#endif
#include <set>
...
namespace umt {
    ...
    template<class T> VDefault inline std::mutex ObjManager<T>::_mtx;

    template<class T> VDefault inline std::unordered_map<std::string, typename ObjManager<T>::wptr> ObjManager<T>::_map;
